...
# Ignore warnings (like feature name mismatch)
...
crop_data["Production"] = crop_data["Production"].fillna(crop_data["Production"].mean())
crop_data["Yield"] = crop_data["Production"] / crop_data["Area"]
crop_data = crop_data.drop(["State_Name"], axis=1)
...
# ------------------------- SAVE MODEL ------------------------- #
# Combine model + feature columns in one dictionary
model_data = {
    "model": model,
    "features": list(X.columns)
}

joblib.dump(model_data, "CY.pkl")

print("\n✅ Model saved successfully as CY.pkl")
